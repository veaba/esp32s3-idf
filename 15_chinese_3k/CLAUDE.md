...
ESP32-S3 firmware project (ATK-DNESP32S3 V1.2 dev board) that drives an SPI LCD display with Chinese character (chinese) rendering. Uses ESP-IDF v5.5.4 build system.
...
The 16x16 Chinese character font data is in `components/BSP/CHINESE16/chinese16.h` (auto-generated, 20902 characters). To regenerate:

```bash
cd ../code_diff
uv run scripts/generate_chinese.py
```
...
│       ├── CHINESE16/          # 16x16 Chinese character font (GB2312, 20902 chars, auto-generated)
│       │   └── chinese16.h     # Unicode index table + 32-byte bitmap per character
...
- Chinese: 16x16 only, from `chinese16.h` (GB2312 Unicode mapping)
- `TextBox` struct provides auto-wrapping text regions

### Chinese Character Rendering

- Font data in `components/BSP/CHINESE16/chinese16.h` contains a Unicode index table (`chinese_unicode_table[]`) and a flat bitmap array (`chinese_bitmap[]`, 32 bytes per char = 16x16 pixels)
- The function `show_chinese_string()` is declared in `chinese16.h` to render Chinese text
- Font generation tool lives in `../code_diff/scripts/generate_chinese.py` (uses `uv run`)
